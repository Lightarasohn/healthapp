...
namespace healthapp.Repositories
{
    public class AppointmentRepository : IAppointmentRepository
    {
        ...
        public async Task<ApiResponse<Appointment>> CreateAppointmentAsync(int patientId, CreateAppointmentDto dto)
        {
            ...
            // DÜZELTME: Email'de doktor adını kullanmak için User tablosunu da Include ediyoruz.
            ...
            if (!string.IsNullOrEmpty(doctor.UnavailableDates))
            {
                ...
                catch (Exception ex) { Console.WriteLine($"UnavailableDates parse error: {ex.Message}"); }
            }
            ...
            try
            {
                ...
                try
                {
                    ...
                    if (patient != null && !string.IsNullOrEmpty(patient.Email))
                    {
                        ...
                        // Email async gönderiliyor ama await ile ana akışı bekletmemek için
                        // Fire-and-forget yapılabilir veya await ile beklenebilir.
                        // Güvenilirlik için await kullanıyoruz.
                        await _emailService.SendEmailAsync(patient.Email, subject, body);
                    }
                }
                catch (Exception ex)
                {
                    // Email hatası randevuyu iptal etmemeli, sadece logla
            ...
            {
                return new ApiResponse<Appointment>(500, $"Randevu oluşturulurken bir hata oluştu: {ex.Message}");
            }
        }
